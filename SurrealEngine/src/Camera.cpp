...
namespace engine_name_placeholder {
	...
	void Camera::onUpdate(GLFWwindow* window)
	{
		...
		if (std::abs(m_oldMouseX - newMousePosX) > 0.1)
		{
			m_yaw += glm::radians(m_oldMouseX - newMousePosX);
			m_yaw = std::fmodf(m_yaw, 2 * 3.1415);
		}
		// old - newMousePos
		if (std::abs(m_oldMouseY - newMousePosY) > 0.1)
		{
	...
	{
		m_viewMatrix = glm::translate(glm::mat4(1.0f), m_camPos) * glm::toMat4(glm::quat(glm::vec3(m_pitch, m_yaw, 0.0f)));
